...
func shouldContinueWithFileExists(l log.Logger, path string, ifExists GenerateConfigExists) (bool, error) {
	...
	case ExistsSkip:
		...
		l.Debugf("The file path %s already exists and if_exists for code generation"+
			" set to \"skip\". Will not regenerate file.", path)
	...
	case ExistsOverwrite:
		...
		// exists.
		l.Debugf("The file path %s already exists and if_exists for code generation"+
			" set to \"overwrite\". Regenerating file.", path)
...
func shouldRemoveWithFileExists(l log.Logger, path string, ifDisable GenerateConfigDisabled) (bool, error) {
	...
	case DisabledSkip:
		...
		l.Debugf("The file path %s already exists and if_disabled for code"+
			" generation set to \"skip\", will not remove file.", path)

		return false, nil
	case DisabledRemove:
		// The file exists and will be removed.
		l.Debugf("The file path %s already exists and if_disabled for code"+
			" generation set to \"remove\", removing file.", path)
...
// RemoteStateConfigToTerraformCode converts the arbitrary map that
// represents a remote state config into HCL code to configure that
// remote state.
func RemoteStateConfigToTerraformCode(
	...
	keyProviderBlock := encryptionBlockBody.AppendNewBlock(
		EncryptionKeyProviderKey,
		[]string{keyProvider, encryptionResourceName},
	)
	keyProviderBlockBody := keyProviderBlock.Body()

	// Append method block
	methodBlock := encryptionBlockBody.AppendNewBlock(
		encryptionMethodKey,
		[]string{encryptionDefaultMethod, encryptionResourceName},
	).Body()
